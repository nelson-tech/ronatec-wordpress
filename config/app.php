...
if (!file_exists(__DIR__ . '/keys.php')) {
	...
	$wp_salts = str_replace('define(', 'App::define(', $keys);
	$more_keys = file_get_contents('https://api.wordpress.org/secret-key/1.1/salt/');
	$fist_line = strtok($more_keys, "\n");
	$jwt_salt = str_replace('define(\'AUTH_KEY\'', 'App::define(\'GRAPHQL_JWT_AUTH_SECRET_KEY\'', $fist_line);
	file_put_contents(__DIR__ . '/keys.php', '<?php use ' . App::class . ";\n" . $wp_salts . $jwt_salt);
}
include __DIR__ . '/keys.php';

// WP Offload Media Credentials
define('AS3CF_SETTINGS', serialize(array(
	'provider' => 'do',
	'access-key-id' => env('SPACES_ACCESS_ID'),
	'secret-access-key' => env('SPACES_SECRET_KEY'),
)));
